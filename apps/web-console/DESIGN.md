...
SocialGrowth Web 运营管理平台是面向海外真实社媒运营专员、技术审计人员的高密度工业级运营中枢（Industrial Command Center）。系统承载 Google Artemis 纯物理真机群控调度、独占式切片素材分发、经验规则与 AI 策略审核、导流短链分析、账号异常核查与授权恢复以及小样本 A/B 策略实验评估。
...
- **业务语义状态色 (Semantic Statuses)**：
  - `status-online (#10B981)`：表示 Artemis 纯真机设备在线、心跳正常、策略在明确授权范围内生效。
  - `status-warning (#F59E0B)`：表示切片素材排他独占锁定、策略待单人审核、账号恢复待人工核查。
  - `status-danger (#EF4444)`：表示账号风控限制、任务执行硬超时自愈强杀、域名被封拦截。
  - `status-experiment (#8B5CF6)`：表示 A/B 策略实验组及比较状态，不预设结果改善。
...
1. **KPI 指标卡片 (MetricCard)**：
   - 包含大号核心数值、趋势百分比徽标、统计口径说明标签（如“实际观察 / 规划假设”“来源与缺失原因”）。
2. **状态徽章 (StatusBadge)**：
...
- **DO**：
  - 数据呈现标注实际来源、观察范围和缺失原因；官方与第三方来源分别说明，不将第三方称为必然完整的兜底。
  - 设备展示已核实的实际真机型号及节点；未取得的信息标为待核实，不把 `Samsung Galaxy S23` 示例当成全部设备事实。
  - A/B 实验展示获批主指标、基线与候选的可比条件、来源、缺失和不确定性；不以统一加权评分强制判定赢家。商业目标、产品能力完成与实际效果分开呈现。
- **DON'T**：
