...
flowchart LR
    ...
    subgraph Phase2 ["阶段二：调度中枢与真机端侧"]
        ...
        P2C["真机配置纯化 (TASK-03)"]
        P2D["冷备换号 2FA 接管协议 (TASK-04)"]
    end

    subgraph Phase3 ["阶段三：外围服务与闭环联调"]
        P3A["短链 302 服务与防封 (TASK-07)"]
...
- **阶段二（调度中枢与真机执行）**：
  ...
  4. 落地冷备换号工单与 2FA 人机接管协议（`waiting_2fa` 状态流转）。
- **阶段三（外围服务与全流程联调）**：
  1. 升级短链服务：FB 独立短链单条归因 + YT 频道主页归因，服务端智能分流爬虫与真实受众。
